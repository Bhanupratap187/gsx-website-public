...
import { StageNavProvider } from "./stage-nav";
...
export function ExperienceMap({ screens, panels, tabBar }: ExperienceMapProps) {
  ...
  // Shared with the controls drawn inside the mockup. Clamped rather than
  // wrapped, so a screen CTA can never send the reader back to stage one.
  ...
  return (
    <StageNavProvider advance={advance}>
      <div className="mt-[clamp(36px,4.5vw,64px)] flex flex-wrap items-stretch gap-[clamp(16px,2vw,32px)]">
        ...
        <div className="shadow-card @container flex min-w-0 flex-[9999_1_470px] flex-col overflow-hidden rounded-[22px] bg-white">
          ...
          <div className="border-line-soft mt-auto grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-4 gap-y-4 border-t px-[clamp(24px,2.4vw,38px)] py-5 @[560px]:flex @[560px]:gap-5">
            <button
              type="button"
              onClick={() => setIndex((i) => (i + last) % STAGES.length)}
              className="border-line-card hover:border-blue order-3 flex min-h-11 flex-none items-center gap-2 justify-self-start rounded-2xl border bg-white px-6 py-3.5 text-base font-bold transition-colors hover:bg-blue-50 @[560px]:order-none"
            >
            ...
            <span
              aria-hidden="true"
              className="bg-line-card rounded-pill order-1 h-[7px] w-full min-w-[70px] overflow-hidden @[560px]:order-none @[560px]:w-auto @[560px]:flex-[1_1_140px]"
            >
              <span
            ...
            <span
              aria-live="polite"
              className="text-muted-2 order-2 flex-none justify-self-end text-base font-semibold @[560px]:order-none"
            >
            ...
            <button
              ...
              className={cn(
                "order-4 flex min-h-11 flex-none items-center gap-2 justify-self-end rounded-2xl px-7 py-4 text-base font-bold transition-colors @[560px]:order-none",
                index === last
